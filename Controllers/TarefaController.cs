...
namespace TrilhaApiDesafio.Controllers
{
    ...
    public class TarefaController : ControllerBase
    {
        ...
        {
            if (!Enum.TryParse(typeof(EnumStatusTarefa), status, true, out var enumStatus))
            return BadRequest(new { Erro = "Status inválido" });

            var tarefas = _context.Tarefas.Where(x => x.Status == (EnumStatusTarefa)enumStatus).ToList();
            return Ok(tarefas);
        }
